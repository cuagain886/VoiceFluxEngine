...
- 🔌 **模型无关适配器。** 切换 ASR / LLM / TTS 全凭配置、不改内核。**LLM** 接任意 OpenAI 兼容 SSE 端点；**TTS** 接 sherpa-onnx **或**任意 OpenAI 兼容 `/audio/speech` 端点；**ASR** 接 sherpa-onnx —— 开源本地 sidecar，且三段都备有内置 mock。详见 [配置](#配置)。
...
配置按此顺序解析、后者覆盖前者：**内置默认 → YAML 文件（`VOICESTREAM_CONFIG`）→ 环境变量覆盖**。什么都不设就是全 mock 默认、监听 `:8080`。密钥**绝不**从文件读取，只走环境变量（启动时自动从 `.env` 加载）。完整带注释的参考配置见 [configs/sherpa.yaml](configs/sherpa.yaml)。

**按配置选模型 —— 内核一行不动。** 每个阶段指名一个适配器，每个适配器读自己那段配置：

| 阶段 | `adapters.*` | 可选值 | 配置段 | 密钥（环境变量） |
|---|---|---|---|---|
| **ASR** | `asr` | `mock` · `sherpa` | `adapters.sherpa` | —（本地 sidecar） |
| **LLM** | `llm` | `mock` · `openai-compat` | `adapters.cloud_llm` | `VOICESTREAM_LLM_API_KEY` |
| **TTS** | `tts` | `mock` · `sherpa` · `openai-tts` | `adapters.sherpa` / `adapters.openai_tts` | `VOICESTREAM_TTS_API_KEY`（仅云端） |

换后端就是一行配置。例如用通用的 `openai-tts` 适配器接云 TTS（OpenAI，或任意兼容 `/audio/speech` 的代理 —— kokoro、edge-tts、自建服务）：

```yaml
adapters:
  tts: openai-tts            # 内核唯一感知到的变化
  openai_tts:
    base_url: "https://api.openai.com/v1"
    model: "tts-1"
    voice: "alloy"
    api_key_env: "VOICESTREAM_TTS_API_KEY"   # key 只在环境变量里，绝不写进文件
```

> ⚠️ **只吃 16 kHz（D11：v1 不做重采样）。** 端点必须回采样率等于 `audio.sample_rate` 的裸 16-bit 单声道 PCM。真实 OpenAI 的 `pcm` 固定 **24 kHz** —— 要么换成可配采样率的兼容服务，要么把 `audio.sample_rate` 设成 24000，否则音高/语速会错。

**环境变量**（全部可选；写进 `.env` 即可 —— 模板见 [.env.example](.env.example)）：

| 变量 | 作用 |
|---|---|
| `VOICESTREAM_CONFIG` | YAML 配置文件路径；不设 = 内置全 mock 默认 |
| `VOICESTREAM_LLM_API_KEY` | `openai-compat` LLM 的 API key（变量名由 `cloud_llm.api_key_env` 指定） |
| `VOICESTREAM_TTS_API_KEY` | `openai-tts` TTS 的 API key（变量名由 `openai_tts.api_key_env` 指定） |
| `VOICESTREAM_ADDR` | 覆盖监听地址（如 `:9090`） |
| `VOICESTREAM_SAMPLE_RATE` | 覆盖线上采样率 |
...
WebSocket + PCM 16k/16-bit/mono。模型可插拔：**LLM** 接任意 OpenAI 兼容 SSE 端点（云端或本地）、**ASR** 接开源 sherpa-onnx（本地 sidecar）、**TTS** 接 sherpa-onnx 或任意 OpenAI 兼容 `/audio/speech` 端点，切换全凭配置、不改内核。FFmpeg 编解码、gRPC/WebTransport 传输拆为未来独立 change。
